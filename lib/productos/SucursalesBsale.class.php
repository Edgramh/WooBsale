 ...
 * @author angelorum
 */
class SucursalesBsale extends BsaleBase
{
 function __construct()
    {
        $p = Funciones::get_value('DB_PREFIX');
        $prefix = Funciones::get_prefix() . $p;
        $table_name = $prefix . Funciones::get_value('DB_TABLE_SUCURSALES_BSALE');
    ...
    public function get_sql_insert($arr_data, $only_header = false, $only_data = false)
    {
        ...
        $address = isset($arr_data['address']) ? $wpdb->_real_escape($arr_data['address']) : '';
        $country = isset($arr_data['country']) ? $wpdb->_real_escape($arr_data['country']) : '';
        $municipality = isset($arr_data['municipality']) ? $wpdb->_real_escape($arr_data['municipality']) : '';
        $city = isset($arr_data['city']) ? $wpdb->_real_escape($arr_data['city']) : '';
        ...
        $sql_header = "INSERT INTO `$table`"
                . "(`id`, `name`, `address`, `country`, `municipality`, `city`, `description`, `isVirtual`, `state`) "
                . "VALUES ";
        ...
        //cuerpo del sql
        $sql_data = " ('$id', '$name', '$address', '$country', '$municipality', '$city', '$description', '$isVirtual', '$state')";
    ...
    /**
     * devuelve array de sucursales suc_id=>suc name
     * @return array
     */
    ...
    public function get_all_sucursales($state = 0, $limit = -1)
    {
        $table = $this->get_table_name();
