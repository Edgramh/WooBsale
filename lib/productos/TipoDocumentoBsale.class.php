 ...
 * @author angelorum
 */
class TipoDocumentoBsale extends BsaleBase
{
  ...
  function __construct()
    {
        ...
        $table_name = $prefix . Funciones::get_value('DB_TABLE_TIPO_DOCUMENTO_BSALE');
    ...
    public function get_sql_insert($arr_data, $only_header = false, $only_data = false)
    {
        ...
        $initialNumber = isset($arr_data['initialNumber']) ? $arr_data['initialNumber'] : '';
        $codeSii = isset($arr_data['codeSii']) ? $wpdb->_real_escape($arr_data['codeSii']) : '';
        $isElectronicDocument = isset($arr_data['isElectronicDocument']) ? $arr_data['isElectronicDocument'] : '-1';
        $breakdownTax = isset($arr_data['breakdownTax']) ? $wpdb->_real_escape($arr_data['breakdownTax']) : '-1';
        $use = isset($arr_data['use']) ? $arr_data['use'] : '-1';
        $isSalesNote = isset($arr_data['isSalesNote']) ? $arr_data['isSalesNote'] : '-1';
        $isExempt = isset($arr_data['isExempt']) ? $wpdb->_real_escape($arr_data['isExempt']) : '-1';
        $restrictsTax = isset($arr_data['restrictsTax']) ? $wpdb->_real_escape($arr_data['restrictsTax']) : '-1';

        $useClient = isset($arr_data['useClient']) ? $wpdb->_real_escape($arr_data['useClient']) : '-1';
        $messageBodyFormat = ''; //isset($arr_data['messageBodyFormat']) ? $wpdb->_real_escape($arr_data['messageBodyFormat']) : '';


        $thermalPrinter = isset($arr_data['thermalPrinter']) ? $wpdb->_real_escape($arr_data['thermalPrinter']) : '-1';

        $state = isset($arr_data['state']) ? $arr_data['state'] : -1;

        $copyNumber = isset($arr_data['copyNumber']) ? $wpdb->_real_escape($arr_data['copyNumber']) : '-1';
        $isCreditNote = isset($arr_data['isCreditNote']) ? $arr_data['isCreditNote'] : '-1';
        $continuedHigh = isset($arr_data['continuedHigh']) ? $wpdb->_real_escape($arr_data['continuedHigh']) : '-1';
        $ledgerAccount = isset($arr_data['ledgerAccount']) ? $wpdb->_real_escape($arr_data['ledgerAccount']) : '-1';
        $ipadPrint = isset($arr_data['ipadPrint']) ? $wpdb->_real_escape($arr_data['ipadPrint']) : '-1';
        $ipadPrintHigh = isset($arr_data['ipadPrintHigh']) ? $wpdb->_real_escape($arr_data['ipadPrintHigh']) : '-1';


        //header
        $sql_header = "INSERT INTO `$table`"
                . "(`id`, `name`, `initialNumber`, `codeSii`, `isElectronicDocument`, `breakdownTax`, "
                . "`use_`, `isSalesNote`, `isExempt`, `restrictsTax`, `useClient`, "
                . "`messageBodyFormat`, `thermalPrinter`, `state`, `copyNumber`, `isCreditNote`, "
                . "`continuedHigh`, `ledgerAccount`, `ipadPrint`, `ipadPrintHigh`) "
                . "VALUES ";
        ...
        //cuerpo del sql
        $sql_data = " ('$id', '$name', '$initialNumber', '$codeSii', '$isElectronicDocument', '$breakdownTax', '$use', "
                . "'$isSalesNote', '$isExempt', '$restrictsTax', '$useClient', 
                 '$messageBodyFormat', '$thermalPrinter', '$state', '$copyNumber', '$isCreditNote', 
                 '$continuedHigh', '$ledgerAccount', '$ipadPrint', '$ipadPrintHigh')";
    ...
    public function get_all_save_in_db($clear_all_before_insert = true)
    {
        ...
        for( $c = 1; $total > 0; $total -= $limit, $c++ )
        {
            ...
            $url = BSALE_TIPO_DOCTOS_URL . "?limit=$limit&offset=$offset";
